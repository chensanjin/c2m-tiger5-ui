...
export default {
  data() {
    return {
      queryParams: {
        planNo: 'SF10086', // 计划单号
        technologyCode: 'pt20210004', // 车间编号
        workshopCode: this.$route.meta.workshopCode, // 工艺编号（例如干混预置填充的编号）
        productCode: 'gangshinaicha', // 产品编号
  ...
  methods: {
        ...
        for (const key in result) {
          if (key === item.INDEX) {
            result[key].forEach(it => {
              if (it.procedureCode === 'GHYZ') {
                if (it.lineCode === M1CODE) {
                  item.m1.yz = it
                } else if (it.lineCode === M2CODE) {
                  item.m2.yz = it
                } else if (it.lineCode === TOTALCODE) {
                  item.total.yz = it
                }
              } else {
                if (it.lineCode === M1CODE) {
    ...
    headMerge({ row, column, rowIndex, columnIndex }) {
      ...
      if (rowIndex === 0 && columnIndex >= 2 && columnIndex <= 9) {
