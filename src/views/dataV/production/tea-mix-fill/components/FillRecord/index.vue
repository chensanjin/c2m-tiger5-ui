 ...
 * @LastEditTime: 2021-08-18 18:24:06
    ...
    <div class="plan-main-content">
      ...
      <ul ref="contentEventRef" class="plan-main-content-event">
        <li v-for="item in OperationRecordList" :key="item.id">
          ...
          <span class="plan-main-content-event-color">{{
            item.createTime
          }}</span>
          <span>进行了</span>
          <span class="plan-main-content-event-color">{{
      ...
      <div class="plan-main-content-button">
        <div class="plan-main-content-button-name" @click="handleDeclare">
          数据填报
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getOperationRecordList() {
      ...
      getOperationRecord({ workshopCode })
        .then(res => {
