 ...
 * @LastEditTime: 2021-07-12 14:53:31
    ...
    <div class="plan-main-content">
      ...
      <ul ref="contentEventRef" class="plan-main-content-event">
        <li v-for="item in OperationRecordList" :key="item.id">
          ...
          <span>进行了</span>
          <span class="plan-main-content-event-color">{{ item.productName }}</span>
          <span>数据填报</span>
        </li>
      </ul>
...
import { getWorkshopRecord } from '@/api/datav'
...
export default {
  ...
  //   currentRow: {
  //     handler(currentRow) {
  //       this.form.capsuleNumber = currentRow.capsuleNumber ? currentRow.capsuleNumber[0] : []
  //       this.form.lineCode = currentRow.lineCode
  //       this.tableData = [currentRow]
  //     },
  //     immediate: true,
  //   },
  // },
  ...
  methods: {
    ...
    getOperationRecordList() {
      ...
      getWorkshopRecord({ workshopCode }).then((res) => {
    ...
    handleDeclare() {
      ...
      // if(planNo){
      if (this.$store.state.tableRow.currentRow.id) {
        this.$refs.declareDialogref.openDialog()
      } else {
        this.$message.error('请选择一行进行填报')
      }
      // }else{
