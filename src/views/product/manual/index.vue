<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['product:manual:edit']"
          type="success"
          plain
          icon="el-icon-edit"
          size="mini"
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['product:manual:remove']"
          type="danger"
          plain
          icon="el-icon-delete"
          size="mini"
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['product:manual:export']"
          type="warning"
          plain
    ...
    <el-table v-loading="loading" :data="manualList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="车间名称" align="center" prop="workshopIdText" />
      <el-table-column label="文件名" align="center" prop="fileName" />
      <el-table-column label="访问地址" align="center" prop="url" />
...
<script>
import { listManual, getManual, delManual, addManual, updateManual, exportManual } from '@/api/product/manual'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除操作手册编号为"' + ids + '"的数据项?', '警告', {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有操作手册数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.exportLoading = true
        return exportManual(queryParams)
      }).then(response => {
